...
This document describes the tokens recognized by the lexer, as defined in `token.h`. Tokens are categorized into keywords, single character tokens, double character tokens, and other tokens. Each token is represented by an enum value in the `_TokenType` enumeration.
...
| `->`        | `TOKEN_ARROW`               | Function return type or lambda arrow     |
| `::`        | `TOKEN_COLON_COLON`         | Namespace or scope resolution            |
